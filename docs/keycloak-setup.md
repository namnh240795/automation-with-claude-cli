...
After configuring identity providers in Keycloak, use the social login API to authenticate users.
...
[
  {
    "provider": "google",
    "name": "Google",
    "authorizationUrl": "http://localhost:8080/realms/app-realm/protocol/openid-connect/auth?kc_idp_hint=google&...",
    "configured": true
  },
  {
    "provider": "github",
    "name": "GitHub",
    "authorizationUrl": "http://localhost:8080/realms/app-realm/protocol/openid-connect/auth?kc_idp_hint=github&...",
    "configured": false
  }
]
```
...
```bash
# For Google
curl -L http://localhost:3001/auth/social/login/google

# For GitHub
curl -L http://localhost:3001/auth/social/login/github?redirect_uri=http://localhost:3001/auth/callback&state=random-state

# For Facebook
curl -L http://localhost:3001/auth/social/login/facebook
```

### Handle Callback

After user authenticates with the provider, Keycloak redirects back with authorization code:

```bash
# Example callback URL
http://localhost:3001/auth/social/callback/google?code=xyz&session_state=abc
```

The API exchanges the code for tokens and returns user information:
...
1. **Get providers** - `GET http://localhost:3001/auth/social/providers`
2. **Click login link** - Copy `authorizationUrl` from response or call `GET /auth/social/login/:provider`
3. **Complete OAuth flow** - Browser opens, login with social provider
4. **Receive tokens** - Callback URL redirects with access token and user info
