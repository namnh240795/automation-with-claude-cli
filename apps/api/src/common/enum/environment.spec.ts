...
describe('ENVIRONMENT', () => {
  ...
  describe('should be readonly', () => {
    it('should maintain const assertion', () => {
      // The object should have readonly properties due to 'as const'
      expect(ENVIRONMENT.SERVICE_PREFIX).toBe('SERVICE_PREFIX');
      expect(ENVIRONMENT.PORT).toBe('PORT');
    });
  ...
  describe('usage patterns', () => {
    it('should be usable with ConfigService', () => {
      ...
      const mockConfigService = {
        get: (key: string) => {
          if (key === ENVIRONMENT.SERVICE_PREFIX) return 'backend';
          if (key === ENVIRONMENT.PORT) return '3000';
          if (key === ENVIRONMENT.JWT_SECRET) return 'secret';
          return null;
        },
      };

      expect(mockConfigService.get(ENVIRONMENT.SERVICE_PREFIX)).toBe('backend');
      expect(mockConfigService.get(ENVIRONMENT.PORT)).toBe('3000');
